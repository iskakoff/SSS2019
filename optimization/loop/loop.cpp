      ...
      for(int j=0;j<n3;++j){
        ...
        for(int p=0;p<n4;++p){
          ...
          for(int t=1;t<n5-1;++t){
            ...
            int ntau2 = t;
            int ntau3 = t+1;
            G_out[i][j][l] += h/3.*sqrtl*(G_in[ntau1][i][j]*boost::math::legendre_p(l,(grid[ntau1]/beta-0.5)*2.) + 
                                       4.0*G_in[ntau2][i][j]*boost::math::legendre_p(l,(grid[ntau2]/beta-0.5)*2.) + 
                                       G_in[ntau3][i][j]*boost::math::legendre_p(l,(grid[ntau3]/beta-0.5)*2.) );
          }
        }
      }
    }
